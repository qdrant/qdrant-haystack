...
class QdrantFilterConverter(BaseFilterConverter):
    ...
    def convert(
        ...
        for item in filter_term:
            ...
            if operator is None:
                raise FilterError("Operator not found in filters")

            if operator in LOGICAL_OPERATORS and "conditions" not in item:
                raise FilterError(f"'conditions' not found for '{operator}'")
            ...
            if operator == "AND":
                ...
                if field is None or value is None:
                    raise FilterError(f"'field' or 'value' not found for '{operator}'")

                must_clauses.extend(
                    self._parse_comparison_operation(
                        comparison_operation=operator, key=field, value=value
                    )
                )
            else:
                raise FilterError(f"Unknown operator {operator} used in filters")
    ...
    def _parse_comparison_operation(
        ...
        if condition_builder is None:
            raise ValueError(f"Unknown operator {comparison_operation} used in filters")
    ...
    def _build_eq_condition(
        self, key: str, value: models.ValueVariants
    ) -> models.Condition:
    ...
    def _build_in_condition(
        self, key: str, value: List[models.ValueVariants]
    ) -> models.Condition:
        if not isinstance(value, list):
            raise FilterError(f"Value {value} is not a list")
    ...
    def _build_lt_condition(self, key: str, value: float) -> models.Condition:
        if not isinstance(value, (int, float)):
            raise FilterError(f"Value {value} is not an int or float")
    ...
    def _build_has_id_condition(
        self, id_values: List[models.ExtendedPointId]
